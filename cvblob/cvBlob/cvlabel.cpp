			...
			for (unsigned int y=0; y<imgIn_height; y++)
			{
				for (unsigned int x=0; x<imgIn_width; x++)
				{
					if (imageIn(x, y))
					{
						...
						if ((!imageOut(x, y))&&((y==0)||(!imageIn(x, y-1))))	// Yet unlabelled and pixel above is clean
						{
							// Found a top (may have multiple) of a new, unlabelled blob
							labeled = true;
							...
							do	// Find and go along contour and store it
							{
								for (unsigned int numAttempts=0; numAttempts<3; numAttempts++)
								{
									...
									else
									{
										if (imageOut(xx, yy) != label)
										{
											...
											// Update blob boundary information
						...
						if ((y+1<imgIn_height)&&(!imageIn(x, y+1))&&(!imageOut(x, y+1)))	// Standing on an internal contour (?)
						{
							labeled = true;
							...
							if (!imageOut(x, y))	// If unlabeled yet
							{
								/*if (!imageOut(x-1, y))
								{
								cerr << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << endl;
								...
								imageOut(x, y) = l;	// Copy label from left neightbour
								numPixels++;
						...
						// Case for internal pixels which have not been labeled yet (?)
